...
export const ApplicationsPage: React.FC = () => {
    ...
    const unsub = onValue(ref(database, 'requests'), (snap) => {
      if (snap.exists()) {
        const list = Object.entries(snap.val())
          ...
          .filter(r => r.hirerUid === profile.uid)
  ...
  const handleStatusChange = async (req: any, status: 'accepted' | 'rejected') => {
    try {
      ...
      setToastMsg(`Application ${status}`);
    } catch (e) {
      console.error(e);
      ...
      <div style={{ padding: 'var(--spacing-md) var(--spacing-xl)', display: 'flex', gap: '10px' }}>
        <button
          ...
          style={{
            ...
            fontSize: '14px',
            cursor: 'pointer',
            backgroundColor: activeTab === 'received' ? 'var(--color-primary)' : 'var(--color-bg-light)',
        ...
        <button
          ...
          style={{
            ...
            fontSize: '14px',
            cursor: 'pointer',
            backgroundColor: activeTab === 'sent' ? 'var(--color-primary)' : 'var(--color-bg-light)',
      ...
      <div className="list-container" style={{ marginTop: 'var(--spacing-sm)' }}>
        {loading ? (
          ...
          filteredRequests.map(req => {
            const initials = req.workerName ? req.workerName.split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2) : '?';
            const avatarColor = getAvatarColor(req.workerUid || '1');
              ...
              <div key={req.id} className="feed-card" style={{ flexDirection: 'column', gap: '8px', padding: '16px' }}>
                ...
                {req.status === 'accepted' && (
                  <div style={{ marginTop: '12px', borderTop: '1px solid var(--color-border)', paddingTop: '12px', display: 'flex', gap: '8px' }}>
                    {req.workerPhoneNumber && (
                      <a
                        href={`tel:${req.workerPhoneNumber}`}
                        style={{
                    ...
                    <button className="btn btn-primary" style={{ flex: 2, height: '36px', padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px' }} onClick={() => {
                      const chatId = [profile?.uid, req.workerUid].sort().join('_');
                      navigate(`/hirer/chat/${chatId}?name=${encodeURIComponent(req.workerName || 'Worker')}`);
                    }}>
                      <MessageSquare size={14} /> Message Worker
                    </button>
                  </div>
                )}
              </div>
